...
function categorizeComponents(files) {
  ...
  const shared = files.filter(
    f => !f.startsWith('Cleaner') && !f.startsWith('Owner')
  );
...
function generateManifestReport() {
  ...
  const serviceFiles = services.files.filter(
    f => f !== 'index.ts' && !f.includes('.test.ts')
  );
  ...
  console.log(
    `   Main: ${screens.files.filter(f => !f.includes('auth/')).length}`
  );
  console.log(
    `   Auth: ${screens.files.filter(f => f.includes('auth/')).length}`
  );
