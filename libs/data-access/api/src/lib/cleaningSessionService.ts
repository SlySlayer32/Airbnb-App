...
export const cleaningSessionService = {
  ...
  async startCleaning(sessionId: string): Promise<CleaningSession> {
    try {
      ...
      await cleaningUpdateService.recordSessionStart(
        sessionId,
        (session as any).properties?.name
      );
  ...
  validateSessionStart(session: any, currentTime: Date): void {
    ...
    if (hour < 11 || hour >= 15) {
      throw new Error(
        'Cleaning sessions must start between 11:00 AM and 3:00 PM'
      );
    }
    ...
    if (
      currentTime < new Date(sessionStart.getTime() - bufferMinutes * 60000)
    ) {
  ...
  validateSessionCompletion(session: any, completionData: any): void {
    ...
    const elapsed =
      (Date.now() - new Date(session.cleaner_started_at).getTime()) /
      (1000 * 60);
    if (elapsed < minDuration) {
      throw new Error(
        'Session must run for at least 30 minutes before completion'
      );
    }
    ...
    if (
      mockPhotosRequired &&
      !completionData.photosComplete &&
      (!completionData.photos || completionData.photos.length === 0)
    ) {
      throw new Error(
        'Photos are required for this session (3+ guests or 3+ rooms)'
      );
    }
    ...
    if (session.checklist_required && !completionData.checklistComplete) {
      throw new Error(
        'Checklist must be completed before session can be finished'
      );
    }
  },
  ...
  async pauseSession(
    ...
    if ((currentSession as any).status !== 'in_progress') {
      throw new Error('Only active sessions can be paused');
    }

    if ((currentSession as any).is_currently_paused) {
  ...
  async resumeSession(sessionId: string): Promise<CleaningSession> {
    ...
    let totalBreakMinutes = (currentSession as any)?.total_break_minutes || 0;
    if ((currentSession as any).cleaner_paused_at) {
      const pauseTime = new Date((currentSession as any).cleaner_paused_at);
      const resumeTime = new Date(now);
      const breakMinutes = Math.floor(
        (resumeTime.getTime() - pauseTime.getTime()) / (1000 * 60)
      );
  ...
  async completeCleaning(
    ...
    try {
      ...
      if (
        (currentSession as any)?.is_currently_paused &&
        (currentSession as any)?.cleaner_paused_at
      ) {
        const pauseTime = new Date((currentSession as any).cleaner_paused_at);
        const completeTime = new Date(now);
        const finalBreakMinutes = Math.floor(
          (completeTime.getTime() - pauseTime.getTime()) / (1000 * 60)
        );
  ...
  async getTodaySessions(): Promise<CleaningSession[]> {
    ...
    if (teamError) {
      // eslint-disable-next-line no-console
      console.warn(
        'Could not get team member ID, subscribing to all sessions:',
        teamError
      );
    ...
    const enhancedSessions = (data || []).map((session: any) => {
      ...
      const isWithinWindow =
        scheduledTime >= cleaningWindowStart &&
        scheduledTime <= cleaningWindowEnd;
      ...
      const estimatedDuration =
        session.properties?.estimated_cleaning_duration || 120; // default 2 hours
      const expectedCompletionTime = new Date(
        scheduledTime.getTime() + estimatedDuration * 60 * 1000
      );
      ...
      return {
        ...
        dashboard_metadata: {
          ...
          status_indicator: hasCompleted
            ? 'completed'
            : hasStarted
              ? 'in_progress'
              : isOverdue
                ? 'overdue'
                : timeUntilStartMinutes <= 30
                  ? 'starting_soon'
                  : 'scheduled',
          // Priority level for sorting/display
          priority_level: isOverdue
            ? 'urgent'
            : timeUntilStartMinutes <= 30
              ? 'high'
              : timeUntilStartMinutes <= 120
                ? 'medium'
                : 'normal',
        },
      } as CleaningSession;
    ...
    return enhancedSessions as CleaningSession[];
  },
  ...
  calculateEffectiveWorkingMinutes(session: CleaningSession): number {
    ...
    const endTime = session.cleaner_completed_at
      ? new Date(session.cleaner_completed_at)
      : new Date();
  ...
  canResumeSession(session: CleaningSession): boolean {
    return (
      session.status === 'in_progress' && session.is_currently_paused === true
    );
  },
  ...
  async handleStatusChangeNotifications(
    ...
    if (
      ...
      (session as any).properties?.owner_id
    ) {
      const shortNotice =
        ((session as any).cancellation_notice_hours || 0) < 24;
      await notificationService.sendToUser(
        (session as any).properties.owner_id,
        'session_cancelled',
        'Cleaning Session Cancelled',
        `${shortNotice ? 'SHORT NOTICE: ' : ''}Cleaning at ${(session as any).properties.name} has been cancelled. Reason: ${(session as any).cancellation_reason}`,
        {
          cleaning_session_id: session.id,
          property_id: (session as any).property_id,
        }
      );
    }
    ...
    if (
      ...
      await notificationService.sendToUser(
        ...
        'cleaning_started',
        'Cleaning Started',
        `Cleaning has started at ${(session as any).properties.name}`,
        {
          cleaning_session_id: session.id,
          property_id: (session as any).property_id,
        }
    ...
    if (
      ...
      await notificationService.sendToUser(
        ...
        'cleaning_completed',
        'Cleaning Completed',
        `Cleaning has been completed at ${(session as any).properties.name}`,
        {
          cleaning_session_id: session.id,
          property_id: (session as any).property_id,
        }
