...
export class BannerStateService {
  static calculateBannerState(context: BannerStateContext): BannerStateResult {
    ...
    if (timeBasedState) return timeBasedState;

    return this.calculateDayState(sessions, currentTime);
  }

  private static calculateActiveSessionState(
    ...
    if (isPaused) {
      ...
      const isLongBreak = pauseDuration > 30;
      return {
        status: 'break',
        ...
        nextAction: isLongBreak
          ? 'Consider resuming session soon'
          : 'Take your time',
    ...
    if (isOverdue) urgencyReason = 'Session significantly overdue';
    else if (isRunningLate) urgencyReason = 'Session running behind schedule';
    ...
    return {
      ...
      nextAction: isOverdue
        ...
        : isRunningLate
          ? 'Focus on completing session'
          : 'Continue cleaning',
      urgencyReason,
    };
  }

  private static calculateNextSessionState(
    session: CleaningSession,
    currentTime: Date
  ): BannerStateResult {
    ...
    if (minutesUntilStart <= 30) {
      return {
        ...
        priority: minutesUntilStart <= 15 ? 'urgent' : 'high',
      };
    }
  ...
  private static calculateTimeBasedState(
    currentTime: Date,
    sessions: CleaningSession[]
  ): BannerStateResult | null {
    ...
    if (timeOfDay >= 480 && timeOfDay < 600) {
      ...
      if (upcoming.length > 0) {
        return {
          status: 'scheduled',
          message: `Good morning! ${upcoming.length} cleaning${upcoming.length > 1 ? 's' : ''} scheduled today`,
          priority: 'medium',
          actionRequired: true,
    ...
    if (timeOfDay >= 900 && timeOfDay < 960) {
      ...
      return {
        status: 'ready',
        message: `${incomplete.length} cleaning${incomplete.length > 1 ? 's' : ''} still pending`,
        priority: 'high',
        actionRequired: true,
  ...
  private static calculateDayState(
    sessions: CleaningSession[],
    currentTime: Date
  ): BannerStateResult {
    const hour = currentTime.getHours();
    ...
    if (hour >= 15 && completed === total && total > 0) {
      return {
        status: 'day_wrap',
        message: `Great work! All ${total} cleaning${total > 1 ? 's' : ''} completed today.`,
        priority: 'low',
      };
    }
    ...
    if (
      hour >= 16 &&
      sessions.every(s => s.status === 'completed' || s.status === 'cancelled')
    ) {
      return {
        status: 'relax',
    ...
    return {
      status: 'relax',
      message:
        'No active cleanings. Check your schedule for upcoming sessions.',
      priority: 'low',
    };
  }
  ...
  private static checkPhotoRequirements(session: CleaningSession): {
    ...
    const requiredCount = Math.min(
      Math.max(basePhotos + guestMultiplier + sizeMultiplier, 2),
      4
    );
  ...
  private static calculatePauseDuration(
    ...
    const currentPauseMinutes = Math.floor(
      (currentTime.getTime() - pauseStart.getTime()) / (1000 * 60)
    );
  ...
  private static isSessionRunningLate(
    session: CleaningSession,
    currentTime: Date
  ): boolean {
    if (!session.dashboard_metadata?.expected_completion_time) return false;
    const expectedTime = new Date(
      session.dashboard_metadata.expected_completion_time
    );
    return currentTime > expectedTime;
  }

  private static isSessionOverdue(
    session: CleaningSession,
    currentTime: Date
  ): boolean {
    if (!session.dashboard_metadata?.expected_completion_time) return false;
  ...
  static getStatusPriority(status: BannerState): number {
    const priorities: Record<BannerState, number> = {
      awaiting_photos: 4,
      active: 3,
      ready: 2,
      break: 2,
  ...
  static validateStateTransition(from: BannerState, to: BannerState): boolean {
    const validTransitions: Record<BannerState, BannerState[]> = {
