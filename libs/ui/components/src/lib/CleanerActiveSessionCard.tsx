...
export default function CleanerActiveSessionCard({
  ...
  const startTime = session.cleaner_started_at
    ? new Date(session.cleaner_started_at)
    : new Date();
  ...
  if (isPaused && session.cleaner_paused_at) {
    ...
    const currentPauseSeconds = Math.floor(
      (currentTime.getTime() - pauseTime.getTime()) / 1000
    );
  ...
  const getProgressPercentage = () => {
    ...
    const estimatedDuration =
      ((session.properties as any)?.estimated_cleaning_duration || 120) * 60; // Convert to seconds
  ...
  const getPhotoRequirements = async () => {
    ...
    try {
      const { PhotoProofService } = await import(
        '../services/photoProofService'
      );
      const hasRequirements = await PhotoProofService.hasPhotoRequirements(
        session.id
      );
  ...
  const canComplete =
    !photoRequirements.required || photoRequirements.completed;
    ...
    <View style={[styles.container, isRunningLate() && styles.containerLate]}>
      ...
      <View style={styles.propertyInfo}>
        <Text style={styles.propertyName}>
          {session.properties?.name || 'Property'}
        </Text>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity
          style={[
            ...
            !canComplete && styles.primaryButtonDisabled,
          ]}
          onPress={() => {
        ...
        >
          ...
          <Text
            style={[
              ...
              !canComplete && styles.primaryButtonTextDisabled,
            ]}
          >
            {photoRequirements.required && !photoRequirements.completed
              ? 'Photos Required'
